...
class GitPoller(base.PollingChangeSource):
    ...
    def startService(self):
        ...
        if not os.path.exists(self.workdir + r'/.git'):
            if self.gitbin == 'git':
                d = self.initGitRepository()
                d.addErrback(log.err, 'while initializing GitPoller repository')
            elif self.gitbin == 'hg':
                d = self.initHgRepository()
                d.addErrback(log.err, 'while initializing GitPoller repository')
        else:
            log.msg("GitPoller repository already exists")

        # call this *after* initGitRepository, so that the initLock is locked first
    ...
    def poll(self):
        if self.gitbin == 'git':
            ...
            d = self._hg_get_changes()
            d.addCallback(self._hg_process_changes)
            d.addErrback(self._hg_process_changes_failure)
            d.addCallback(self._catch_up)
            d.addErrback(self._catch_up_failure) 
        return d
    ...
    def _get_hg_changes(self):
        log.msg('gitpoller: polling hg repo at %s' % self.repourl)

        self.lastPoll = time.time()
        
        # get a deferred object that performs the fetch
        args = ['pull', self.branch]
        self._extend_with_fetch_refspec(args)
